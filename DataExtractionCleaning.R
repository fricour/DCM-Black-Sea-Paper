# Author : Florian Ricour (July 7 2020)
#
# This script extracts data from ARGO data files and applies quality controls to biogeochemical variables such as Chlorophyll a (Chla), 
# Photosynthetic Available Radiation (PAR), etc.
#
...
#4 Function to normalize data 
normalize <- function(data){
  data <- (data - min(data, na.rm = T))/(max(data, na.rm = T) - min(data, na.rm = T))
}

#5 Function to QC PAR data (from Louis Terrats - Villefranche Oceanographic Laboratory - related article https://journals.ametsoc.org/doi/full/10.1175/JTECH-D-15-0193.1)
...
profiles <- ldply(as.list(filename),function(file){
  ...
  if (file == "7900591_Mprof2020.nc" | file == "7900592_Mprof2020.nc"){ 
...
profiles <- profiles[!(profiles$year %in% c(2013,2020)),]
...
argodf <- ddply(profiles,~juld,summarize,
                depthmin = depth[which.max(fluo):length(fluo)][which.min(fluo[which.max(fluo):length(fluo)])],# to not take into account the surface where Chla can be minimum
...
crit2 <- argodf[(argodf$bottomdepth < 40),] 

# remove a profile that does not answer to the script (due to a max in Chla at more the 150 m depth, which is not possible in the Black Sea) 
...
MLDdf <- ldply(as.list(unique(density_profiles$juld)), function(i){ # Script adapted from (and thanks to) MARIN CORNEC - Villefranche Oceanographic Laboratory
...
profiles$fluo <- profiles$fluo * 0.65
...
# Put negative Chla concentrations (remaining after all corrections) to 0
...
doxy <- ldply(as.list(c("6900807_Mprof2020.nc","6901866_Mprof2020.nc","7900591_Mprof2020.nc","7900592_Mprof2020.nc","6903240_Mprof2020.nc")),function(file){
...
# DENSITY PROFILES WITH CHLA AND BBP # (merged in the M-files but this may be solvable with the new S-files?)
######################################

# Let's create a dataframe based on potential density anomaly, CHLA and BBP
