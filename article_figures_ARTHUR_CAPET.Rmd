...
interpolated_light_data <- ldply(as.list(unique(complete_tmp2$juld)), function(i){
  tmp1 <- complete_tmp2[complete_tmp2$juld == i,]
  tmp2 <- initPAR[initPAR$juld == i,]
  if(nrow(tmp2) == 0){
    absolute_light <- rep(NA, nrow(tmp1))
    # relative % of incoming surface PAR
    relative_light <- absolute_light
  }else{
    absolute_light <- approx(tmp2$depth, tmp2$par, tmp1$depth)$y
    # compute relative % of incoming surface PAR
    relative_light <- (absolute_light/tmp2$par[1])*100 # could also put max(tmp2$par) car surface = max_par, therefore no ordering issue will appear (but tmp2$par[1] is good) 
  }
  data.frame(depth = tmp1$depth, absolute_light = absolute_light, relative_light = relative_light)
})

# add it to complete_tmp2
complete_tmp2 <- cbind(complete_tmp2, light = interpolated_light_data$relative_light, light_absolute = interpolated_light_data$absolute_light)
#complete_tmp2 <- cbind(complete_tmp2, light = interpolated_light_data$absolute_light)
...
integrated_diags <- ddply(complete_tmp2, .(juld,platform), summarize,
                          ...
                          # Adding diags for LIGHT Horizon here
                            ISOlow       = try_default(approx(x=depth, y=light, xout=ZLow, rule=2)$y,NA, quiet = FALSE),
                            ISO90B       = try_default(approx(x=depth, y=light, xout=Z90B, rule=2)$y,NA, quiet = FALSE),
                            ISO90T       = try_default(approx(x=depth, y=light, xout=Z90T, rule=2)$y,NA, quiet = FALSE),
                            ISOMLD       = try_default(approx(x=depth, y=light, xout=unique(MLD), rule=2)$y,NA, quiet = FALSE),
                            ISOMAX       = try_default(approx(x=depth, y=light, xout=unique(Zmax), rule=2)$y,NA, quiet = FALSE),
                            ISODEMI      = try_default(approx(x=depth, y=light, xout=unique(Zdemi), rule=2)$y,NA, quiet = FALSE),
                            ISO_iso1 = try_default(approx(x=depth, y=light, xout=unique(Ziso1b), rule=2)$y,NA, quiet = FALSE),
...
dbdf$sigma_norm_layers <- cut(dbdf$sigma_norm,seq(0.05,3.05,.05))
dbdf$depth_norm_layers <- cut(dbdf$depth_norm,seq(0.05,3.05,.05))
```
...
bigplot1<-ggplot(subset(blbl, month %in% c(3,4,5,6)), aes(x=depth_norm, y=value))+
  ...
  theme_bw()+coord_flip(xlim = c(0,2), ylim=c(0,2))+
...
tmp_iso1 <- subset(tmp_fitco, select = c('month','ISOMAX','ISOlow','ISO90B','ISO90T','dcm_or_not','year','shape'))

tmp_iso1$sigmaMAX[tmp_iso1$dcm_or_not==0]<-NA
tmp_iso1$sigmaMAX[tmp_iso1$Zmax>200]<-NA
...
#              fun.data = mean_cl_boot,
  ...
  geom_boxplot(aes(y=value, group=interaction(month,variable)), alpha=0.2, notch = TRUE)+
        scale_x_continuous(NULL, labels = month.abb, breaks = seq(1,12)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  #scale_y_continuous(trans = 'log10') + ylab('[microMoleQuanta/m^2/sec]') +
  ylab('% of Incoming PAR') + theme_bw()  + scale_y_continuous(trans = 'log10') +
      # scale_color_brewer(aesthetics = c("colour", "fill"),type = qual ,palette=MyPal,
      #                  name="ISO Horizons", 
      ...
      #                           TeX('$\\ISO_{50,bottom}$'),
      #                           TeX('$\\ISO_{50,top}$')))#,
      #                           #TeX('$\\ISO_{\\mathrm{PAR}\\,1%}$')))
      scale_color_manual(aesthetics = c("colour", "fill"),values = c("#D95F02","#7570B3","#E7298A","#66A61E"), # [1] "#1B9E77" "#D95F02" "#7570B3" "#E7298A" "#66A61E" "#E6AB02" "#A6761D" "#666666"
                       name="ISO Horizons", 
                       labels=list(TeX('$\\ISO_{DCM}$'),
                                TeX('$\\ISO_{low}$'),
                                TeX('$\\ISO_{50,bottom}$'),
                                TeX('$\\ISO_{50,top}$')))
...
#absolute light
tmp_iso1 <- subset(tmp_fitco, select = c('month','ISOaMAX','ISOalow','ISOa90B','ISOa90T','dcm_or_not','year','shape'))
...
#              fun.data = mean_cl_boot,
  ...
  geom_boxplot(aes(y=value, group=interaction(month,variable)), alpha=0.2, notch = TRUE)+
        scale_x_continuous(NULL, labels = month.abb, breaks = seq(1,12)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  scale_y_continuous(trans = 'log10') + ylab('[microMoleQuanta/m^2/sec]') + theme_bw()+
  #ylab('% of Incoming PAR') + theme_bw()  + scale_y_continuous(trans = 'log10') +
      # scale_color_brewer(aesthetics = c("colour", "fill"),type = qual ,palette=MyPal,
      #                  name="ISO Horizons", 
      #                  labels=list(TeX('$\\ISO_{DCM}$'),
