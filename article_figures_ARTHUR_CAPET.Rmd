...
integrated_diags <- ddply(complete_tmp2, .(juld,platform), summarize,
                          ...
                          # Add ABSOLUTE Light levels
                            ISOalow       = try_default(approx(x=depth, y=absolute_light, xout=ZLow, rule=2)$y,NA, quiet = FALSE),
                            ISOa90B       = try_default(approx(x=depth, y=absolute_light, xout=Z90B, rule=2)$y,NA, quiet = FALSE),
                            ISOa90T       = try_default(approx(x=depth, y=absolute_light, xout=Z90T, rule=2)$y,NA, quiet = FALSE),
                            ISOaMLD       = try_default(approx(x=depth, y=absolute_light, xout=unique(MLD), rule=2)$y,NA, quiet = FALSE),
                            ISOaMAX       = try_default(approx(x=depth, y=absolute_light, xout=unique(Zmax), rule=2)$y,NA, quiet = FALSE),
                            ISOaDEMI      = try_default(approx(x=depth, y=absolute_light, xout=unique(Zdemi), rule=2)$y,NA, quiet = FALSE),
                            ISOa_iso1     = try_default(approx(x=depth, y=absolute_light, xout=unique(Ziso1b), rule=2)$y,NA, quiet = FALSE)
                          )
}
```
...
```{r Fig10, fig.width=8, fig.height=6}
...
depthplot <- ggplot(tmp_zmax, aes(x=month)) +
    geom_smooth(aes(y=value, fill=variable, color=variable),stat = 'summary',
#              fun.data = median_hilow,
             fun.data = mean_cl_boot,
              fun.args = list(conf.int = 0.95), alpha=0.5)+
  geom_boxplot(aes(y=value, fill=variable, color=variable, group=interaction(month,variable)), alpha=0.2, notch = TRUE)+
        scale_x_continuous(NULL, labels = month.abb, breaks = seq(1,12), limits = c(0.5,12.5), expand = c(0, 0)) +
...
densplot <- ggplot(tmp_sig, aes(x=month)) +
    geom_smooth(aes(y=value, fill=variable, color=variable),stat = 'summary',
#              fun.data = median_hilow,
              fun.data = mean_cl_boot,
              fun.args = list(conf.int = 0.95), alpha=0.5)+
  geom_boxplot(aes(y=value, fill=variable, color=variable,group=interaction(month,variable)), alpha=0.2, notch = TRUE)+
        scale_x_continuous(NULL, labels = month.abb, breaks = seq(1,12), expand = c(1, 1)*0.01) +
  theme_bw()+
  theme(panel.grid.minor.x = element_line(colour = "grey"),
        panel.grid.major.x = element_line(colour = "white")) +
  ylab(TeX('$\\left[kg\\,m^{-3}\\right]$$'))+coord_cartesian(ylim=c(16,10))+scale_y_reverse()+
...
absolute_light <- ggplot(tmp_iso, aes(x=month)) +
    geom_smooth(aes(y=value, fill=variable, color=variable),stat = 'summary',
#              fun.data = median_hilow,
              fun.data = mean_cl_boot,
              fun.args = list(conf.int = 0.95), alpha=0.5)+
  ...
  ylab(TeX('$\\left[\\mu mol\\,photons\\,m^{-2}\\,s^{-1}\\right]$')) +
  scale_color_manual(aesthetics = c("colour", "fill"),
                     values = c("#1B9E77", "#D95F02", "#7570B3", "#E7298A", "#66A61E"),
                       name="PAR Horizons",
                      labels=list(TeX('$\\mathrm{PAR$_{MLD}$'),
                                   TeX('$\\mathrm{PAR}_{DCM}$'),
                                   TeX('$\\mathrm{PAR}_{low}$'),
                                   TeX('$\\mathrm{PAR}_{50,bottom}$'),
                                   TeX('$\\mathrm{PAR}_{50,top}$')))
...
```{r FigR5, fig.width=10, fig.height=10}
gA <- ggplotGrob(depthplot+
  geom_text(data=data.frame(x=1, y=0, label='a)'), aes(x=x, y=y, label=label), color='black', fill='black', size=5))
gB <- ggplotGrob(densplot+
  geom_text(data=data.frame(x=1, y=10, label='b)'), aes(x=x, y=y, label=label), color='black', fill='black', size=5))
gC <- ggplotGrob(absolute_light+
  geom_text(data=data.frame(x=1, y=300, label='c)'), aes(x=x, y=y, label=label), color='black', fill='black', size=5))
grid::grid.newpage()
...
partplot <- ggplot(tmp_part, aes(x=month)) +
    geom_smooth(aes(y=value, fill=variable, color=variable),stat = 'summary',
#              fun.data = median_hilow,
              fun.data = mean_cl_boot,
              fun.args = list(conf.int = 0.95), alpha=0.5)+
  ...
  theme(panel.grid.minor.x = element_line(colour = "grey"),
    ...
    geom_boxplot(aes(y=value, fill=variable, color=variable, group=interaction(month,variable)), alpha=0.2)+
  ylab('[%]')+
...
concplot<-  ggplot(tmp_conc, aes(x=month)) +
      geom_smooth(aes(y=value, fill=variable, color=variable),stat = 'summary',
  #              fun.data = median_hilow,
                fun.data = mean_cl_boot,
                fun.args = list(conf.int = 0.95), alpha=0.5)+
          scale_x_continuous(NULL, labels = month.abb, breaks = seq(1,12), expand = c(0, 0)) +
        theme_bw()+
  theme(panel.grid.minor.x = element_line(colour = "grey"),
        panel.grid.major.x = element_line(colour = "white")) +
      geom_boxplot(aes(y=value, fill=variable, color=variable, group=interaction(month,variable)), alpha=0.2)+
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ylab(TeX('$\\left[mg\\,m^{-3}\\right]$$'))+coord_cartesian(ylim=c(0,2.5))+
    scale_color_manual(aesthetics = c("colour", "fill"),
                     values = c("#1B9E77", "#7570B3","#D95F02"),#,  "#E7298A", "#66A61E"),
                       name="Chla concentration", 
                       labels=c("Total (scaled)","Surface","DCM"))

concplot
```

```{r FigR6, fig.width=10, fig.height=6}
gA <- ggplotGrob(concplot+
  geom_text(data=data.frame(x=1, y=2.5, label='a)'), aes(x=x, y=y, label=label), color='black', fill='black', size=5))
gB <- ggplotGrob(partplot+
  geom_text(data=data.frame(x=1, y=100, label='b)'), aes(x=x, y=y, label=label), color='black', fill='black', size=5))
...
```{r Fig11, fig.width=10, fig.height=3}
...
ms1<-ms+geom_point(data=tmp_fitco, aes(x=lon,y=lat, color=factor(dcm_or_not)), alpha=.6)+
  facet_wrap(~monthn, scale="fixed")+
  theme_bw()+
  #scale_color_discrete(name="", labels=c('non-DCM','DCM'))+
  scale_color_manual(values = c("black", "red"), labels=c('non-DCM','DCM'), name="")+
  ...
  # for another wrap colour (B/W instead of grey)
  theme(panel.background=element_rect(fill='white', colour='black'),strip.background=element_rect(fill='white', colour='black'))
...
P_years_depth <- ggplot(subset(tmp_zmax, variable !="Zdemi"), aes(x=month, fill=variable, color=variable)) +
  ...
  scale_x_continuous(name='', breaks=c(), labels="", limits=c(.5,12.5), expand = c(0,0))+
  ylab('[m]')+theme_bw()+
...
P_years_sig <-ggplot(tmp_sig, aes(x=month, fill=variable, color=variable)) +
  ...
  ylab(TeX('$\\left[kg\\,m^{-3}\\right]$$'))+
  coord_cartesian(ylim=c(16,10), expand = FALSE)+
  theme_bw()+
  #scale_color_brewer(aesthetics = c("colour", "fill"),type = qual ,palette=MyPal)+
  scale_color_brewer(aesthetics = c("colour", "fill"),type = qual ,palette=MyPal,
                       name="Density Horizons : ", 
...
```{r inter_light, include=FALSE}
#absolute light
tmp_isoC<- dcast(tmp_iso,formula = month~variable, fun.aggregate = median, na.rm=TRUE)
tmp_isoA<-melt(tmp_isoC, id.vars=c('month'))

P_years_iso <-ggplot(tmp_iso, aes(x=month, fill=variable, color=variable)) +
       geom_line(data=tmp_isoA,aes(y=value),alpha =1, size=0.5, linetype=2)+
      geom_smooth(aes(y=value, group=interaction(year, variable)),stat = 'summary',
             fun.data = mean_cl_boot,
              fun.args = list(conf.int = 0.95), 
                  alpha=0.2, linetype=0)+
  scale_x_continuous(name='', breaks=c(), labels="", limits=c(.5,12.5))+
  ylab(TeX('$\\left[\\mu mol\\,photons\\,m^{-2}\\,s^{-1}\\right]$'))+theme_bw()+
    scale_y_continuous(trans = 'log10') +
  #scale_color_brewer(aesthetics = c("colour", "fill"),type = qual ,palette=MyPal)+
  scale_color_brewer(aesthetics = c("colour", "fill"),type = qual ,palette=MyPal,
                       name="PAR Horizons : ", 
...
```{r Fig12, fig.height=12, fig.width=10}
grid.arrange(P_years_depth,
             P_years_sig+theme( strip.background = element_blank(),strip.text.x = element_blank()), 
             P_years_iso+theme( strip.background = element_blank(),strip.text.x = element_blank()), 
